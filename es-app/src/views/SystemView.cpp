...
namespace
{
	struct SystemCarouselExtras
	{
		...
		// Pulso suave del sistema central/seleccionado.
		// El tiempo queda local a SystemView y no requiere tocar el header.
		bool selectedLogoPulse = false;
		float selectedLogoPulseTime = 0.0f;

		// Pulso de opacidad del logo central, configurado en <carousel>.
		bool logoOpacityPulse = false;
		float logoOpacityPulseMin = 0.65f;
		float logoOpacityPulseMax = 1.0f;
		float logoOpacityPulseDuration = 1600.0f;
		float logoOpacityPulseTime = 0.0f;
	};
...
void SystemView::refreshMostPlayedVideo()
{
	if (!mMostPlayedVideo)
		return;

	// Detener y limpiar siempre el sistema anterior antes de cargar el nuevo.
	mMostPlayedVideo->onHide();
	mMostPlayedVideo->topWindow(false);
	mMostPlayedVideo->setVideo("");
	mMostPlayedVideo->setImage("");
	mMostPlayedVideo->setVisible(false);
	...
	SystemData* system = mEntries.at(mCursor).object;
	if (!system)
		return;
	...
	// No es extra="true": es un único reproductor compartido por SystemView.
	const ThemeData::ThemeElement* elem =
		theme->getElement("system", "mostPlayedVideo", "video");

	if (!elem)
		return;

	using namespace ThemeFlags;
	mMostPlayedVideo->applyTheme(
		theme,
		"system",
		"mostPlayedVideo",
		POSITION | SIZE | DELAY | Z_INDEX | ROTATION | VISIBLE);

	const bool themeVisible = mMostPlayedVideo->isVisible();
	const std::string imagePath = system->getMostPlayedImage();
	const std::string videoPath = system->getMostPlayedVideo();

	mMostPlayedVideo->setImage(imagePath);
	const bool hasVideo = mMostPlayedVideo->setVideo(videoPath);
	const bool hasImage = !imagePath.empty();

	mMostPlayedVideo->setVisible(themeVisible && (hasVideo || hasImage));

	if (mMostPlayedVideo->isVisible())
	{
		// El reproductor no es child de SystemView, por eso debemos
		// propagar manualmente el estado de ventana activa.
		mMostPlayedVideo->topWindow(true);
		mMostPlayedVideo->onShow();
	}
}
	...
	mInfoFocus(false)
{
	...
	if (Settings::getInstance()->getBool("VideoOmxPlayer"))
		mMostPlayedVideo.reset(new VideoPlayerComponent(window, ""));
	else
		mMostPlayedVideo.reset(new VideoVlcComponent(window, ""));
#else
	mMostPlayedVideo.reset(new VideoVlcComponent(window, ""));
#endif

	mMostPlayedVideo->setDefaultZIndex(10);
	mMostPlayedVideo->setVisible(false);
...
void SystemView::update(int deltaTime)
{
	...
	if (mMostPlayedVideo)
		mMostPlayedVideo->update(deltaTime);

	// Pulso del sistema central: avanza únicamente cuando el carrusel está
	// completamente detenido. Al desplazarse se reinicia para evitar saltos.
	SystemCarouselExtras& carouselExtras = getCarouselExtras(this);
	const bool carouselStable =
		(getScrollingVelocity() == 0) &&
		(std::fabs(mCamOffset - static_cast<float>(mCursor)) < 0.001f);

	if (mShowing && carouselExtras.selectedLogoPulse && carouselStable)
	{
		carouselExtras.selectedLogoPulseTime += static_cast<float>(deltaTime) / 1000.0f;

		// Mantener el valor pequeño para evitar pérdida de precisión tras muchas horas.
		if (carouselExtras.selectedLogoPulseTime >= 1.5f)
			carouselExtras.selectedLogoPulseTime =
				std::fmod(carouselExtras.selectedLogoPulseTime, 1.5f);
	}
	else
	{
		carouselExtras.selectedLogoPulseTime = 0.0f;
	}

	// ES-X: tiempo del pulso de opacidad del logo central.
	if (mShowing && carouselExtras.logoOpacityPulse && carouselStable)
	{
		carouselExtras.logoOpacityPulseTime += static_cast<float>(deltaTime);

		const float duration =
			std::max(100.0f, carouselExtras.logoOpacityPulseDuration);

		if (carouselExtras.logoOpacityPulseTime >= duration)
			carouselExtras.logoOpacityPulseTime =
				std::fmod(carouselExtras.logoOpacityPulseTime, duration);
	}
	else
	{
		carouselExtras.logoOpacityPulseTime = 0.0f;
	}
...
void SystemView::onCursorChanged(const CursorState& /*state*/)
{
	...
	// El nuevo sistema seleccionado comienza el pulso desde reposo.
	getCarouselExtras(this).selectedLogoPulseTime = 0.0f;
	getCarouselExtras(this).logoOpacityPulseTime = 0.0f;

	// Al cambiar sistema, volver el foco al carrusel y refrescar visibilidad del botón
	setInfoFocus(false);

	// Cambiar la imagen/video al juego más jugado del nuevo sistema.
	refreshMostPlayedVideo();
...
void SystemView::renderCarousel(const Transform4x4f& trans)
{
	...
	auto renderLogo = [this, &carouselTrans, &logoSpacing, xOff, yOff, &extraCfg](int i)
	{
		...
		if(selected)
		{
			...
			// Pulso tipo PS3: el elemento central sube suavemente y vuelve
			// a su posición. No se aplica durante el desplazamiento.
			const bool carouselStable =
				(getScrollingVelocity() == 0) &&
				(std::fabs(mCamOffset - static_cast<float>(mCursor)) < 0.001f);

			if(extraCfg.selectedLogoPulse && carouselStable)
			{
				constexpr float pulseDuration = 1.5f;
				constexpr float twoPi = 6.28318530718f;
				const float phase =
					(extraCfg.selectedLogoPulseTime / pulseDuration) * twoPi;

				// 0 -> 1 -> 0: movimiento elegante, sin bajar de la posición base.
				const float pulse = 0.5f - (0.5f * std::cos(phase));
				const float amplitude = 0.003f * mSize.y();
				baseY -= pulse * amplitude;
			}
		}
		...
		// ES-X: pulso de opacidad únicamente en el logo central.
		if(selected && extraCfg.logoOpacityPulse)
		{
			const bool carouselStable =
				(getScrollingVelocity() == 0) &&
				(std::fabs(mCamOffset - static_cast<float>(mCursor)) < 0.001f);
			...
			{
				const float duration =
					std::max(100.0f, extraCfg.logoOpacityPulseDuration);
				const float phase =
					(extraCfg.logoOpacityPulseTime / duration) * 6.28318530718f;
				const float wave = 0.5f - (0.5f * std::cos(phase));
				const float factor =
					extraCfg.logoOpacityPulseMin +
					(extraCfg.logoOpacityPulseMax - extraCfg.logoOpacityPulseMin) * wave;

				opacity = (int)Math::round(static_cast<float>(opacity) * factor);
			}
		}
...
void SystemView::renderExtras(const Transform4x4f& trans, float lower, float upper)
{
	...
	const int firstPosition = extrasCenter + logoBuffersLeft[bufferIndex];
	const int lastPosition  = extrasCenter + logoBuffersRight[bufferIndex];

	// Con pocos sistemas, un mismo índice puede repetirse por el carrusel circular.
	// Elegimos únicamente la copia más cercana a la cámara para el reproductor.
	int videoPosition = std::numeric_limits<int>::min();
	float bestVideoDistance = std::numeric_limits<float>::max();

	if (mMostPlayedVideo && mMostPlayedVideo->isVisible())
	{
		for (int i = firstPosition; i <= lastPosition; i++)
		{
			...
			if (index == mCursor)
			{
				const float distance = std::fabs((float)i - mExtrasCamOffset);
				if (distance < bestVideoDistance)
				{
	...
	for(int i = firstPosition; i <= lastPosition; i++)
	{
		...
		if(mShowing || index == mCursor)
		{
			...
			const bool renderVideoHere =
				(i == videoPosition) &&
				mMostPlayedVideo &&
				mMostPlayedVideo->isVisible() &&
				mMostPlayedVideo->getZIndex() >= lower &&
				mMostPlayedVideo->getZIndex() < upper;
			...
			for (GuiComponent* extra : renderComponents)
			{
				...
				if (renderVideoHere &&
				    !videoRendered &&
				    mMostPlayedVideo->getZIndex() <= extra->getZIndex())
				{
					mMostPlayedVideo->render(extrasTrans);
					videoRendered = true;
				}
			...
			if (renderVideoHere && !videoRendered)
				mMostPlayedVideo->render(extrasTrans);
...
void SystemView::getDefaultElements(void)
{
	...
	extraCfg.selectedLogoPulse = false;
	extraCfg.selectedLogoPulseTime = 0.0f;
	extraCfg.logoOpacityPulse = false;
	extraCfg.logoOpacityPulseMin = 0.65f;
	extraCfg.logoOpacityPulseMax = 1.0f;
	extraCfg.logoOpacityPulseDuration = 1600.0f;
	extraCfg.logoOpacityPulseTime = 0.0f;
...
void SystemView::getCarouselFromTheme(const ThemeData::ThemeElement* elem)
{
	...
	if(elem->has("selectedLogoPulse"))
		extraCfg.selectedLogoPulse = elem->get<bool>("selectedLogoPulse");

	// Pulso de opacidad del logo central (propiedades del carousel).
	if(elem->has("pulse"))
		extraCfg.logoOpacityPulse = elem->get<bool>("pulse");

	if(elem->has("pulseOpacityMin"))
		extraCfg.logoOpacityPulseMin =
			std::max(0.0f, std::min(1.0f, elem->get<float>("pulseOpacityMin")));

	if(elem->has("pulseOpacityMax"))
		extraCfg.logoOpacityPulseMax =
			std::max(0.0f, std::min(1.0f, elem->get<float>("pulseOpacityMax")));

	if(extraCfg.logoOpacityPulseMin > extraCfg.logoOpacityPulseMax)
		std::swap(extraCfg.logoOpacityPulseMin, extraCfg.logoOpacityPulseMax);

	if(elem->has("pulseDuration"))
		extraCfg.logoOpacityPulseDuration =
			std::max(100.0f, elem->get<float>("pulseDuration"));
